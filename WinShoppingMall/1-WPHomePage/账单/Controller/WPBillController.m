...
//        NSIndexPath *index = [NSIndexPath indexPathForRow:self.billArray.count - 1 inSection:0];
...
- (void)getBillListData {
    ...
    [WPHelpTool getWithURL:self.isCheck ? WPCheckBillURL : WPBillURL parameters:parameters success:^(id success) {
        ...
        if ([type isEqualToString:@"1"]) {
            ...
            weakSelf.incomeString = self.isCheck ? qr_income : income;
            weakSelf.expensesString = result[@"expenses"];
            NSString *titleStr = self.isCheck ? [NSString stringWithFormat:@"%@\n收入 ¥%@", self.dateString.length > 0 ? [NSString stringWithFormat:@"%@年%@月", self.year, self.month] : @"全部账单", weakSelf.incomeString] : [NSString stringWithFormat:@"%@\n收入 ¥%@, 支出 ¥%@", self.dateString.length > 0 ? [NSString stringWithFormat:@"%@年%@月", self.year, self.month] : @"全部账单", weakSelf.incomeString, weakSelf.expensesString];
            [_titleView.titleButton setTitle:titleStr forState:UIControlStateNormal];
            [weakSelf titleView];
        }
        else {

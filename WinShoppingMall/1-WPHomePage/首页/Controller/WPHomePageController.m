...
- (void)initCreditCard
{
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(kScreenWidth / 2 + 20, (80 - (kScreenWidth / 2 - 40) * 0.3) / 2, kScreenWidth / 2 - 40, (kScreenWidth / 2 - 40) * 0.3)];
...
- (void)topClassButtonClick:(UIButton *)button{
    switch (button.tag) {
        case 0: {
            // 充值
            WPRechargeViewController *vc = [[WPRechargeViewController alloc] init];
            [self.navigationController pushViewController:vc animated:YES];
        }
            break;
        case 1: {
...
//            [self.navigationController pushViewController:vc animated:YES];
        ...
        case 6: {
            // 转账
            if ([[WPUserInfor sharedWPUserInfor].approvePassType isEqualToString:@"YES"]) {
                WPTransferAccountsController *vc = [[WPTransferAccountsController alloc] init];
                [self.navigationController pushViewController:vc animated:YES];
            }
            else {
                [WPProgressHUD showInfoWithStatus:@"请您先完成实名认证"];
...
- (void)creditCardButtonClick {
    if ([[WPUserInfor sharedWPUserInfor].approvePassType isEqualToString:@"YES"]) {
        WPCreditCardPaymentController *vc = [[WPCreditCardPaymentController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
    }
    else {
        [WPProgressHUD showInfoWithStatus:@"请您先完成实名认证"];

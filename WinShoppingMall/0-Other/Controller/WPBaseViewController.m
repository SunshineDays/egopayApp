...
- (UIActivityIndicatorView *)indicatorView
{
    if (!_indicatorView) {
        ...
        _indicatorView.color = [UIColor themeColor];
...
- (UIAlertController *)alertCtr {
    if (!_alertCtr) {
        _alertCtr = [UIAlertController alertControllerWithTitle:@"请在系统设置中开启权限（设置>易购付>相机/相册>开启）" message:nil preferredStyle:UIAlertControllerStyleAlert];
        [_alertCtr addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//                [self.navigationController popViewControllerAnimated:YES];
        }]];;
        [_alertCtr addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (UIAlertController *)alertSheet {
    if (!_alertSheet) {
        ...
        UIAlertAction *selectPicAction = [UIAlertAction actionWithTitle:@"从相册中选择" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//              AVAuthorizationStatus authStatus = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
//              if (authStatus == AVAuthorizationStatusRestricted && !AVAuthorizationStatusNotDetermined) {
//                  [weakSelf presentViewController:weakSelf.alertCtr animated:YES completion:nil];
//              }
//              else {
...
//              }
        ...
        if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
            [_alertSheet addAction:takePicAction];
        }
        [_alertSheet addAction:selectPicAction];
        [_alertSheet addAction:cancelAction];
    }
    return _alertSheet;
}
...
- (UIAlertController *)alertCameraSheet
{
    if (!_alertCameraSheet) {
        ...
        [_alertCameraSheet addAction:takePicAction];
        [_alertCameraSheet addAction:cancelAction];
    }

...
#import "WPEditInforController.h"
#import "WPPickerView.h"


@interface WPAddCardController () <UIPopoverPresentationControllerDelegate, UITextFieldDelegate, WPPickerViewDelegate>
...
- (void)bankCellClick:(UIButton *)button
{
    ...
    vc.selecteNameBlock = ^(NSString *nameStr) {
        if ([nameStr isEqualToString:@"其他银行"]) {
            WPEditInforController *vc = [[WPEditInforController alloc] init];
...
- (void)dateButtonClick:(UIButton *)button
{
    ...
    WPPickerView *pickerView = [[WPPickerView alloc] initPickerView];
...
//    else if ([self.cardType isEqualToString:@"1"] && [self.yearNow isEqualToString:self.yearSelected] && ([self.monthSelected integerValue] < [self.monthNow integerValue])) {
//        [WPProgressHUD showInfoWithStatus:@"有效期时间不能早于当前时间"];
//    }
    else {
...
{
    [WPProgressHUD showProgressWithStatus:@"提交中，请稍后..."];
